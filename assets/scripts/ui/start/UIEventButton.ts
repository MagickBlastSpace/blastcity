...
export class UIEventButton extends Component {
		...
		if (this.progressLabel) {
			if (this.eventController.getIsComplete()) {
				...
				this.progressLabel.string =
					this.eventController.getCollectable() +
					'/' +
					this.eventController.getCurrentStageStep();
			}
		}
	}
	...
	refresh() {
		...
		if (this.warningIndicator) {
			this.warningIndicator.active =
				this.eventController.isRewardAvailable() &&
				!this.eventController.getIsChecked();
		}
		if (this.countIndicator) {
			this.countIndicator.active =
				this.eventController.getIsChecked() &&
				this.eventController.getRewardsCount() > 0;
			this.countLabel.string = String(this.eventController.getRewardsCount());
		}
	...
	showEventPrefab() {
		let isPortrait =
			ResolutionManager.instance.isPortraitOrientation() &&
			this.isPortraitVersionAvailable;
	...
	getPosition(): Vec2 {
		...
		if (parent && uiTransform) {
			...
			if (parentUITransform) {
				worldPosition = parentUITransform.convertToWorldSpaceAR(
			...
			console.error(
				'UITransform component is missing on this node or node has no parent',
			);
		}
		...
		bundlesToLoad.forEach((bundleName) => {
			bundle.load(bundleName, Prefab, (err, prefab) => {
				...
				if (bundleName === this.eventName) {
					this.eventPopup = instantiatedNode.getComponent(
						'UIEvent' + this.eventName,
					);
					this.eventPopup.init(this.eventController);
				} else {
					this.eventPopup_portrait = instantiatedNode.getComponent(
						'UIEvent' + this.eventName,
					);
