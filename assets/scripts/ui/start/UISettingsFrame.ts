...
import { DebugGold } from '../../data/DebugGold';
...
export class UISettingsFrame extends UIPopupFrameBase {
	...
	start() {
		...
		this.langBtn_Ru.node.on(
			Button.EventType.CLICK,
			this.onLangBtnClickRu,
			this,
		);
		this.langBtn_En.node.on(
			Button.EventType.CLICK,
			this.onLangBtnClickEn,
			this,
		);
	}
	...
	async onToggleDevMode(toggle: Toggle) {
		const isDebug = toggle.isChecked;

		UserData.instance.setDevMode(isDebug);

		console.log('Developer mode is now:', isDebug);

		this.bp.setDebugMode(isDebug);
		this.debugConstructor.setDebugMode(isDebug);

		if (isDebug) {
			await DebugGold.enable();
		} else {
			await DebugGold.disable();
		}
	}
